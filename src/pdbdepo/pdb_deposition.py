...
def process_input(
    ...
    for index, row in df.iterrows():
        ...
        if meta_data is None:
            ...
            cif_doc.write_file(str(xtal_out_dir / 'original.cif'))
            ...
            refine_item = find_loop_item(cif_doc[0], '_refine')
            if refine_item:
                append_loop_items(cif_doc[0], refine_item, mmcifgen_refine_tags, mmcifgen_refine_values)

            # add in the common metadata (generated by mmcif-gen)
            for item in mmcifgen_block:
                cif_doc[0].add_item(item)
            ...
            cif_doc.write_file(str(xtal_out_dir / 'structure.cif'))
...
def run(collator_dir, metadata_cif, output_dir, logger):
...
def main():
    ...
    run(args.collator_dir, args.metadata_cif, args.output_dir, logger)
