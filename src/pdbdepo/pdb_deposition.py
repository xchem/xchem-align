    ...
    with open(aimless_p, "rt") as f:
        ...
        for line in f:
            ...
            if match:
                ...
                info('found aimless version', aimless_ver)
                return aimless_ver
    return None
...
def read_pairs(block, prefix, erase: bool):
    """
    Read the pairs with this prefix and generate a dict of tags and values
    :param block:
    :param prefix:
    :param erase: Whether to erase the pairs
    :return:
    """
    d = {}
    for item in block:
...
def read_phasing_software(xtal_dir):
    ...
    if dimple_log_file_path.is_file():
        with open(dimple_log_file_path, 'rt') as dimple:
            ...
            for line in dimple:
                ...
                if looking:
                    ...
                    if match:
                        info('found dimple version', match.group(1))
                        dimple_ver = match.group(1)
                        break
                    match = re.search(r'^\[\w+\]$', line)
                    if match:
                        info('dimple version not found')
                        break
                else:
                    if line == '[workflow]':
                        looking = True
        ...
        warn('dimple log file not found:', str(dimple_log_file_path))
...
def read_fragalysis_csv(filename):
    """Read Fragalysis metadata CSV and return a dict mapping crystal name -> list of pose codes.

    The CSV has pose code in column 1 (index 0) and crystal/experiment code in column 3 (index 2).
    """
    d = {}
    if filename:
        with open(filename, 'rt', newline='') as f:
            ...
            for tokens in reader:
                if len(tokens) >= 3:
                    ...
                    if crystal_name not in d:
                        d[crystal_name] = []
                    d[crystal_name].append(pose_code)
    info('read pose IDs for', len(d), 'crystals')
    return d
...
def run(
    ...
    pose_ids_dict = read_fragalysis_csv(fragalysis_csv)
...
def main():
    ...
    parser.add_argument("-f", "--fragalysis-csv", help="CSV file with short codes from Fragalysis download")
    ...
    LOG = utils.create_singleton_logger(str(logfile_p), args.log_level)
    ...
    run(
        ...
        fragalysis_csv=args.fragalysis_csv,
        debug=args.debug,
    )
